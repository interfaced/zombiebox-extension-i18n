...
export default class Service {
	...
	_findValueInPacks(locale, key) {
		...
		reduceLocale(locale, (reducedLocale) => {
			(this._packs[reducedLocale] || []).some((pack) => {
				const packValue = pack.getValue(key);
				if (typeof packValue === 'string') {
					foundValue = packValue;
					return true;
				}
				return false;
			});

			return typeof foundValue === 'string';
		});
