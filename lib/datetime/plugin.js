...
zb.i18n.datetime.Plugin = class {
	...
	relative(date, now = new Date(), options = {}) {
		...
		return this._context.transKey(`relative.${unit}.${tense}`, {value});
	}
	...
	_getFormatString(type, form) {
		...
		const locale = zb.i18n.utils.reduceLocale(
			this._context.getLocale(),
			(locale) => Formats.hasOwnProperty(locale)
		);

		const fallbackLocale = zb.i18n.utils.reduceLocale(
			this._context.getFallbackLocale(),
			(locale) => Formats.hasOwnProperty(locale)
		);
