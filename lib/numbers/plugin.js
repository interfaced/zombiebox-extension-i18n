...
zb.i18n.numbers.Plugin = class {
	...
	isLocaleSupported(locale) {
		return zb.i18n.numbers.data.formats.hasOwnProperty(locale);
	}
	...
	formatCurrency(value, currency, opt_fractional, opt_symbolType = zb.i18n.numbers.CurrencySymbolType.NARROW) {
		...
		const fallbackLocale = context.getFallbackLocale();

		function get(symbolType, locale) {
			const keyString = `numbers.currencies.${currency}.${symbolType}`;
			return context.hasKey(keyString, locale) ? context.transKey(keyString) : null;
		}
		...
		return context.transKey('numbers.formats.currency', {
	...
	_addLocalePack(locale) {
		this._context.addPack(locale, new zb.i18n.Pack({
			'numbers': {
				'formats': zb.i18n.numbers.data.formats[locale],
				'currencies': zb.i18n.numbers.data.currencies[locale]
			}
		}));
	}
};
