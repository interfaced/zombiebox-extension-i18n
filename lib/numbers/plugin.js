...
zb.i18n.numbers.Plugin = class {
	...
	formatNumber(value, opt_precision) {
		...
		if (isGroupingRequired) {
			const hasGroupDelimiter = this._context.hasKey('numbers.formats.delimiters.group', locale);
		...
		if (fractionalPartRequired) {
			const hasDecimalDelimiter = this._context.hasKey('numbers.formats.delimiters.decimal', locale);
